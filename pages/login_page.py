...
class LoginPage(BasePage):
    ...
    def should_be_login_url(self):
        assert "login" in  self.browser.current_url, "URL does not have a login atribute"

    def should_be_login_form(self):
        # реализуйте проверку, что есть форма логина
        assert self.is_element_present(*LoginPageLocators.LOGIN_FORM), 'Can`t find login form'

    def should_be_register_form(self):
        # реализуйте проверку, что есть форма регистрации на странице
        assert self.is_element_present(*LoginPageLocators.REGISTER_FORM), 'Can`t find register form'
